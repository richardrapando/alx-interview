...
class NQueen:
    ...
    def place(self, k, i):
        """ Function that checks if k Queen can be placed in a i column (True)
        or if the are attacking queens in a row or a diagonal (False)
        """
    ...
    def nQueen(self, k):
        """ Function that tries to place every queen in the board
        Args:
        k: the starting queen from which to evaluate (should be 1)
        """
